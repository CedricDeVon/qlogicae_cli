
</br>

<div style="width: 100%; display: flex; justify-content: center;">
    <image src="./qlogicae/qlogicae/application/templates/default/build/qlogicae/application/assets/application.ico" style="width: 128px;">
</div>

</br>



<div style="text-align: center;">
  <h1>QLogicae CLI</h1>
  <p style="font-style: italic;">A Multi-purpose C++ Software Development Tool</p>
<div style="margin: 32px 64px;">
    
![Version](https://img.shields.io/badge/Version-1.0.0-blue)
![GTest](https://img.shields.io/badge/GTest-NA-green)
![Windows](https://img.shields.io/badge/OS-Windows-blue)
![MIT](https://img.shields.io/badge/License-MIT-red)

![x64](https://img.shields.io/badge/Architecture-x64-yellow)
![C](https://img.shields.io/badge/Language-C-white)
![C++](https://img.shields.io/badge/Language-C++20-white)

<!--
![iOS](https://img.shields.io/badge/OS-IOS-white)
![Linux](https://img.shields.io/badge/OS-Linux-yellow)

![x86](https://img.shields.io/badge/Architecture-x86-yellow)
![x64 MASM](https://img.shields.io/badge/Language-MASM-white)
![x86 MASM](https://img.shields.io/badge/Language-MASM-white)

[![Gmail](https://img.shields.io/badge/Gmail-Visit-red)](sample@gmail.com)
[![GitHub](https://img.shields.io/badge/GitHub-Visit-white?logo=github)](https://github.com/sample)
[![Facebook](https://img.shields.io/badge/Facebook-Visit-blue?logo=facebook)](https://facebook.com/sample)
[![Twitter](https://img.shields.io/badge/Twitter-Visit-black?logo=x)](https://twitter.com/sample)
[![Discord](https://img.shields.io/badge/Discord-Visit-indigo?logo=discord)](https://discord.gg/sample)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Visit-blue)](https://linkedin.com/in/sample)
[![Slack](https://img.shields.io/badge/Slack-Visit-green?logo=slack)](https://sample.slack.com)
[![Website](https://img.shields.io/badge/Website-Visit-white)](https://sample.com)
-->

  </div>
</div>

</br>



<h2>📚 Table of Contents</h2>
<ul>
  <li><a href="#about">About</a></li>
  <ul>
    <li><a href="#about-description">Description</a></li>
    <li><a href="#about-core-features">Core Features</a></li>
  </ul>
  <li><a href="#getting-started">Getting Started</a>
    <ul>
      <li><a href="#getting-started-pre-requisites">Pre-requisites</a></li>
    </ul>
  </li>
  <li><a href="./qlogicae/qlogicae/application/documentation/index.md">Extended Documentation</a></li>  
</ul>

</br>



<h2 id="about">
  📖 About
</h2>
<h3 id="about-description">
  🧾 Description
</h3>
<p>
  <strong>QLogicae CLI</strong> is a multi-purpose C++ software development tool with a purpose - make C++ development great again (as if it always was).
  
</p>
<h3 id="about-core-features">
  ⚙️ Core Features
</h3>
<ul>
  <li>
    <h3>QLogicae Project Manager</h3>
    <ul>
      <li><p style="font-weight: bold">Public and Private Configuration Systems</p>
      <li><p style="font-weight: bold">Multi-Environment Build Systems</p>
      <li><p style="font-weight: bold">Windows Registry Handling</p>
      <li><p style="font-weight: bold">Environment Variable Handling</p>
      <li><p style="font-weight: bold">Logging Systems</p>
      <li><p style="font-weight: bold">And More</p>
    </ul>
  </li>
  <li>
    <h3>QLogicae-VS2022 Integrated Automation</h3>
    <ul>
      <li><p style="font-weight: bold">QLogicae Opera Integrated Setup</p>
      <li><p style="font-weight: bold">Visual Studio 2022 Build Process</p>
      <li><p style="font-weight: bold">Application Deployment Via Inno Setup</p>
      <li><p style="font-weight: bold">And more</p>
    </ul>    
  </li>
</ul>

</br>


<h2 id="getting-started">
  🚀 Getting Started
</h2>
<h3 id="getting-started-pre-requisites">
  📋 Pre-requisites
</h3>
<p>
  In order to ensure the most stable user-experience as possible, please meet the requirements below:
</p>
<ul>
  <li><p>OS - Windows 10 (Or Higher)</p></li>
  <li><p>System Architecture - x64</p></li>
  <li><p>Minimum Storage - At least 4 MB</p></li>
  <li><p>Minimum RAM - At least 1 GB</p></li>
  <li><p>PowerShell - 7.5.3.0 (Or Higher)</p></li>
  <li><p>Inno Setup - 6.4.3 (Or Higher)</p></li>
</ul>

</br>


<h2 id="extended-documentation">
  🗂️ Extended Documentation
</h2>
<p>
  If you would like to know more, click the link below.
</p>
<a href="./qlogicae/qlogicae/application/documentation/index.md">🔗 Navigate To Page</a>

</br>
